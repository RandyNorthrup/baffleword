...
public sealed class MainMenuViewModelTests
{
    ...
    [Fact]
    public void PlayBigBoardCommand_SetsGameModeAndNavigates()
    {
        _sut.PlayBigBoardCommand.Execute(null);

        _settingsRepo.Verify(r => r.SetAsync("GameMode", "BigBoard"), Times.Once);
        _navigation.Verify(n => n.NavigateTo<GameViewModel>(), Times.Once);
    }

    [Fact]
    public void PlaySuperBoardCommand_SetsGameModeAndNavigates()
    {
        _sut.PlaySuperBoardCommand.Execute(null);

        _settingsRepo.Verify(r => r.SetAsync("GameMode", "SuperBoard"), Times.Once);
        _navigation.Verify(n => n.NavigateTo<GameViewModel>(), Times.Once);
    }
    ...
    public void AllCommands_AreNotNull()
    {
        ...
        _sut.PlayBigBoardCommand.Should().NotBeNull();
        _sut.PlaySuperBoardCommand.Should().NotBeNull();
