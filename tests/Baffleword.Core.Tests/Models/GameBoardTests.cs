...
public sealed class GameBoardTests
{
    ...
    [Fact]
    public void GetNeighbors_ReturnsCachedCollection()
    {
        GameBoard board = CreateTestBoard();
        BoardCell center = board[1, 1];

        IReadOnlyList<BoardCell> first = board.GetNeighbors(center);
        IReadOnlyList<BoardCell> second = board.GetNeighbors(center);
