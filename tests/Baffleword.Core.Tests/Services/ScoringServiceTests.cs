...
public sealed class ScoringServiceTests
{
    ...
    public void CalculateWordScore_SuperBoard_ReturnsCorrectPoints(string word, int expectedPoints)
    {
        _sut.CalculateWordScore(word, GameMode.SuperBoard).Should().Be(expectedPoints);
    }

    [Theory]
    [InlineData("CAT", 1)] // Standard scoring unchanged
    [InlineData("CATCH", 2)]
    public void CalculateWordScore_BigBoard_UsesSameAsStandard(string word, int expectedPoints)
    {
        _sut.CalculateWordScore(word, GameMode.BigBoard).Should().Be(expectedPoints);
    }
}
