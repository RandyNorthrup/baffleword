...
public sealed class AchievementServiceTests
{
    ...
    [Fact]
    public void GetAllAchievements_DoesNotExposeLegacyBrandName()
    {
        IReadOnlyList<Achievement> achievements = _sut.GetAllAchievements();

        achievements.Should().OnlyContain(a => !a.Description.Contains("Baffleword", StringComparison.OrdinalIgnoreCase));
    }
