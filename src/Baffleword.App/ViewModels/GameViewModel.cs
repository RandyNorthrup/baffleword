...
public sealed class GameViewModel : ViewModelBase, IDisposable
{
    ...
    private Thickness _tileMarginThickness = new(4);
        ...
        ILogger<GameViewModel> logger)
    {
        ...
        FoundWords.CollectionChanged += (_, _) => RefreshGroupedFoundWords();
    ...
    /// </summary>
    public string TimeRemainingText => $"{(int)TimeRemaining.TotalMinutes}:{TimeRemaining.Seconds:00}";
    ...
    /// <summary>
    /// Gets the found words grouped by length.
    /// </summary>
    public ObservableCollection<FoundWordGroup> GroupedFoundWords { get; } = [];
    ...
    public double TileMargin
    {
        ...
        private set
        {
            if (SetProperty(ref _tileMargin, value))
            {
                TileMarginThickness = new Thickness(value);
            }
        }
    }

    /// <summary>
    /// Gets the tile margin as a thickness.
    /// </summary>
    public Thickness TileMarginThickness
    {
        get => _tileMarginThickness;
        private set => SetProperty(ref _tileMarginThickness, value);
    }
    ...
    public void EndDragSelection()
    {
        ...
        if (_selectedPath.Count > 0 && _currentRound?.State == GameRoundState.Playing && !IsPaused)
        {
    ...
    private void SortFoundWords()
    {
        List<WordResult> sorted = FoundWords
            .OrderByDescending(word => word.WordLength)
            .ThenBy(word => word.Word, StringComparer.OrdinalIgnoreCase)
            .ToList();

        FoundWords.Clear();
        foreach (WordResult word in sorted)
        {
            FoundWords.Add(word);
        }
    }

    private void RefreshGroupedFoundWords()
    {
        List<FoundWordGroup> groups = FoundWords
            .GroupBy(word => word.WordLength)
            .OrderByDescending(group => group.Key)
            .Select(group => new FoundWordGroup(
                group.Key,
                group.OrderBy(word => word.Word, StringComparer.OrdinalIgnoreCase).ToList()))
            .ToList();

        GroupedFoundWords.Clear();
        foreach (FoundWordGroup group in groups)
        {
            GroupedFoundWords.Add(group);
        }
    }
