...
public sealed class SettingsViewModel : ViewModelBase, IDisposable
{
        ...
        set
        {
            if (!_disposed && !_audioManager.SoundEffects.IsMuted)
            {
                ...
                if (SetProperty(ref _sfxVolume, clamped))
                {
                    ...
                    OnPropertyChanged(nameof(CanAdjustSfxVolume));
                }
            }
        }
    }
        ...
        set
        {
            if (!_disposed && !_audioManager.Music.IsMuted)
            {
                ...
                if (SetProperty(ref _musicVolume, clamped))
                {
                    ...
                    OnPropertyChanged(nameof(CanAdjustMusicVolume));
                }
            }
        }
    }
    ...
    /// <summary>
    /// Gets a value indicating whether the SFX volume slider can be adjusted.
    /// </summary>
    public bool CanAdjustSfxVolume => !IsSfxMuted;

    /// <summary>
    /// Gets a value indicating whether the music volume slider can be adjusted.
    /// </summary>
    public bool CanAdjustMusicVolume => !IsMusicMuted;
